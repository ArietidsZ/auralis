...
1. 确认 D-Parity：是否接受「Must = V1–V3 + A1–A3，硬件/Space 后置」  
2. 启动 Phase 0 三个 PoC（ASR 流式方言、Hy-MT 量化对照、TTS RTF）  
3. 开工程 Phase 1：打包 `libhymt_jni.so` + TTS 可听 + 双语时间线  
4. 并行建 `:data-library` 空壳与 Records UI，避免引擎通了却无处沉淀会话  
